...
class App extends Component {
    ...
    solveSudoku(){
        if (sudoku.solve(this.state.board)) {
            ...
            this.setState({error: "NIEPOPRAWNE NUMERY :-("});
        }
    }
    ...
    checkSudoku(){
        if (this.state.board !== this.state.initialBoard) {
            if (this.state.board === sudoku.solve(this.state.board)) {
                this.setState({error: "WYGRANA"});
            } else if (sudoku.solve(this.state.board)) {
                this.setState({error: "DO TEJ PORY OK :-)"});
            } else {
                this.setState({error: "NIEPOPRAWNE NUMERY :-("});
            }
        }   
    ...
    render() {
        return (
            <div>
                ...
                />
                {
                    this.state.board ? <button onClick={() => this.undoHandler()} className="undo-button">COFNIJ</button> : null
                }
            </div>
        );
    }
